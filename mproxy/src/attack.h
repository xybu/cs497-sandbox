/**
 * attack.h
 * Defines prototypes for attack list actions.
 * 
 ...
 */
EXTERNC int normalize_ofp_ver_str(char *v);
 ...
 */
EXTERNC int normalize_ofp_msg_type(int ver, char *s);
 ...
 */
EXTERNC int normalize_action_type(char *s);

/**
 * Initialize attacker data structure.
 * Return 0 if succeeds, and 1 otherwise.
 */
EXTERNC int attacker_init();

/**
 * Read attack list row by row.
 */
EXTERNC void attacker_read_rows(char *fpath);
 ...
 * @return 0 if succeeds, and 1 otherwise.
 */
EXTERNC int attacker_add_rule(int id, char *data, size_t len);
